...
async function processRaptorQPacket(
  ...
  try {
    ...
    if (preprocessed) {
      await completeVerifiedRaptorQPayload(current, preprocessed);
    }
  } catch (err: any) {
...
async function completeVerifiedRaptorQPayload(
  state: Pick<DecodeState, 'isText' | 'isCompressed' | 'completed'>,
  preprocessed: Uint8Array,
): Promise<void> {
  let envelopeBytes: Uint8Array;
  try {
    envelopeBytes = state.isCompressed ? inflateSync(preprocessed) : preprocessed;
  } catch {
    self.postMessage({ type: 'error', message: 'Decompression failed — data may be corrupted' });
    return;
  }

  try {
    const verified = await unwrapAndVerifyIntegrityEnvelope(envelopeBytes);
    if (verified.isText !== state.isText) throw new Error('Integrity envelope kind mismatch');
    if (verified.isText) {
      const text = new TextDecoder('utf-8', { fatal: true }).decode(verified.data);
      self.postMessage({ type:'complete', isText:true, text, sha256:verified.sha256, autoStop:true });
    } else {
      const output = verified.data;
      self.postMessage({
        type:'complete', isText:false, data:output.buffer, filename:verified.filename || 'recovered-file',
        mime:verified.mime, sha256:verified.sha256, autoStop:true,
      }, { transfer:[output.buffer as ArrayBuffer] });
    }
    state.completed = true;
  } catch (err: any) {
    self.postMessage({ type:'error', message:`Final SHA-256 verification failed: ${err.message ?? String(err)}` });
  }
}

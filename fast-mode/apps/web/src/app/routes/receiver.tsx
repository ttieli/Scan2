...
import { openPreferredCamera, type CameraFacing } from '@/lib/camera_constraints';
...
export function ReceiverPage() {
  ...
  const [rejectedPackets, setRejectedPackets] = useState(0);
  ...
  const [verifiedSha256, setVerifiedSha256] = useState('');
  ...
  function createWorker(): Worker {
    ...
    w.onmessage = (e: MessageEvent) => {
      ...
      switch (msg.type) {
        ...
        case 'rejected': {
          setRejectedPackets((count) => count + 1);
          break;
        }
        case 'error': {
  ...
  const startCameraScanning = useCallback(async () => {
    ...
    try {
      const stream = await openPreferredCamera(navigator.mediaDevices, cameraFacing, streamRef.current);
    ...
    <div>
      ...
      {inputMode === 'camera' && (
        <div style={S.section}>
          ...
          <div style={{ ...S.toggleGroup, marginBottom: 14 }}>
            <button
              type="button"
              disabled={scanning}
              style={S.toggleBtn(cameraFacing === 'environment')}
            ...
            <button
              type="button"
              disabled={scanning}
              style={S.toggleBtn(cameraFacing === 'user')}
          ...
          >
            <video ref={videoRef} style={{ width: '100%', borderRadius: 6, background: '#000', display: 'block', transform: cameraFacing === 'user' ? 'scaleX(-1)' : 'none' }} playsInline muted />
            {/* Scan-region overlay */}
            <div
              style={{
                position: 'absolute',
          ...
          {scanning && (
            <div style={S.statsBar}>
              ...
              {rejectedPackets > 0 && (
                <span>
                  rejected <span style={S.statValue}>{rejectedPackets}</span>
                </span>
              )}
              <span>
      ...
      {receivedFile && (
        <div style={S.section} ref={fileResultRef}>
          ...
          {verifiedSha256 && (
            <p style={{ margin: '6px 0', color: '#3fb950', fontSize: 13 }}>
              SHA-256 ✓ {verifiedSha256.slice(0, 16)}…
            </p>
          )}
